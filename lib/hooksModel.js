...
module.exports = (Sequelize, patches) => {
	...
	function* afterCreate(item, options) {
		const model = this, // eslint-disable-line no-invalid-this
			{
				primaryKey, foreignKey, levelFieldName, through: throughWoSchema, throughKey, throughForeignKey
			} = model.hierarchy,
			values = item.dataValues,
			parentId = valueFilteredByFields(foreignKey, item, options);

		const through = throughWoSchema.schema(model.getTableName().schema)
	...
	function* _beforeUpdate(item, options) {
		...
		if (level !== oldLevel) {
			...
			`);
			const schema = model.getTableName().schema;
			sql = replaceTableNames(sql, {item: model,  through: schema ? through.schema(schema) : through}, sequelize);
		...
		if (oldParentId !== null) {
			...
			sql = replaceTableNames(sql, {through: schema ? through.schema(schema) : through}, sequelize);
			sql = replaceFieldNames(sql, {itemId: throughKey, ancestorId: throughForeignKey}, through);
